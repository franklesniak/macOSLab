...
- Markdown links to repo-internal files (for example, `SECURITY.md`, `CONTRIBUTING.md`, `CODE_OF_CONDUCT.md`, `README.md`) **MUST** use full absolute URLs of the form `https://github.com/franklesniak/macOSLab/blob/HEAD/<path>`. The old template placeholder workflow has been removed; authors MUST NOT introduce live `OWNER/REPO` placeholder URLs into this initialized repository.
...
- If the repository is ever migrated to a different host or owner/name, update these absolute URLs as part of that migration.
